...
1. **Bug?** Write the failing test first. Confirm it fails. Then either fix it (if trivial) or hand off to `lead-engineer` with the test as the spec.
...
- Don't mock the world to make a test pass — if mocking gets baroque, the design is wrong; flag to `lead-engineer`.
