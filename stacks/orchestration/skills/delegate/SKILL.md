...
- **Sub-agent commits are unverified by design.** Under the canonical `commit.gpgsign=false` recipe they are unsigned and GitHub shows **no badge** (a neutral state — signing with a key GitHub cannot check would instead show the yellow "Unverified"). Making them "Verified" means registering each `bot+<slug>@…` alias on the bot's GitHub account, which **relinks every agent to one profile and one avatar**: per-agent avatars and verified sub-agent commits are mutually exclusive, and this toolkit recommends the avatars. A repo with **required signatures** branch protection cannot use plus-addressed sub-agent authors at all.
