...
Glob the harness **agents** directory — the `agents/` directory beside `{{harness.skillsDir}}` (take `{{harness.skillsDir}}`, swap the trailing `skills` segment for `agents`, and match `*.md`). Under Claude that resolves to `.claude/agents/*.md`.
