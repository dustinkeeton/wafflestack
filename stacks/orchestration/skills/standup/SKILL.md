---
name: standup
description: Round up a quick status pulse from every installed agent — each reports in ≤3 lines, strictly from its own role's seat, where the {{project.name}} code stands and where it's headed. Discovers the roster at runtime, fans out one read-only parallel wave, and prints a compact digest — no fixes, no board writes.
user-invocable: true
...
Glob the harness **agents** directory — the `agents/` directory beside `{{harness.skillsDir}}` (take `{{harness.skillsDir}}`, swap the trailing `skills` segment for `agents`, and match `*.md`). Under Claude that resolves to `.claude/agents/*.md`; under codex and agents-dir, to `.agents/agents/*.md`.
...
> **Standup — `<name>`.** You are the `<name>` agent (<description>). Give an **extremely brief** status pulse on the {{project.name}} codebase **strictly from your own role's perspective** — the view from your seat, nothing else.
>
