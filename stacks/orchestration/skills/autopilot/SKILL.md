...
1. **Re-wait for green, then run one fresh evidence pass.** The final round pushed fixes, so first return to step 1's green wait (if that push left CI red, this is the red-round failure case — stop and report per [Failure handling](#failure-handling); no fresh pass, and never arm a red PR). Then run `qa <pr>` **once more, outside the loop** — it posts its review to the PR as usual (same `<!-- waffle-qa -->` marker) — purely as the source of the escape-hatch brief below. **No `pr-response` follows it**: this pass is evidence, not another fix round, and it does not count toward the cap — the gate spends at most cap+1 `qa` invocations, still strictly bounded. Without it the brief would capture the *last round's* findings, which that same round's `pr-response` already fixed — a stale hand-off (#234).
