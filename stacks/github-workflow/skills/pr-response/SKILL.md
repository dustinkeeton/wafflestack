...
user-invocable: true
argument-hint: "<PR#, number, or PR URL> (omit for the current branch's PR)  [{{prResponse.confirmGate.flag.off}} | {{prResponse.confirmGate.flag.on}}]"
---
...
| `{{prResponse.confirmGate.flag.off}}` (with or without a PR ref) | Skip the confirmation gate before applying fixes — see [The gate convention](#the-gate-convention). |
| `{{prResponse.confirmGate.flag.on}}` (with or without a PR ref) | Force the confirmation gate for this run, even when the rendered gate is off. Both tokens at once is a contradiction — say so and stop. |
...
map, rendered from the stack: `{{prResponse.confirmGate.flag.off}}` skips the gate for one run, `{{prResponse.confirmGate.flag.on}}` forces it for one
run, and either token beats the config value.

**Rendered gate for this repo: `{{prResponse.confirmGate}}`** — the value after `.waffle/waffle.local.yaml` →
...
| `false` | No gate: proceed as if `{{prResponse.confirmGate.flag.off}}` were passed. | Skip. |
| `prompt` | Assume nothing: ask, which for a gate means gating exactly as `true` does. | Skip — the same `nonInteractive: false` fallback. |

`{{prResponse.confirmGate.flag.off}}` exists for an **agent calling this skill** — a delegate answering the review on the PR
...
confirm". `{{prResponse.confirmGate.flag.on}}` is for a repo whose config turned the gate off and a caller who wants this one
...
PR number (or lets it default to the current branch's PR) and `{{prResponse.confirmGate.flag.off}}` to skip the confirmation
