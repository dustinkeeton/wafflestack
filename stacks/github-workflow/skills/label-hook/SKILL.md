...
|---|---|---|
| `{{labelHook.enrichLabel}}` | `enrich` | Enrich issue #N in place — the issue skill's enrich mode |
| `{{labelHook.implementLabel}}` | `implement` | Implement issue #N and open a PR — per the git-workflow skill |
...
1. Fetch #N and its context:
   ...
   ```
   If the issue carries `{{issue.reassessLabel}}`, it is held for human re-evaluation:
   comment that the implement run declined because the issue is labeled
   `{{issue.reassessLabel}}` and needs reconfirmation first (removing the label is that
...
- Never apply '{{labelHook.enrichLabel}}' or '{{labelHook.implementLabel}}' to any
  issue — a hook run must not be able to fan out new hook runs.
- Never add or remove '{{issue.reassessLabel}}' — it is a human's hold, and only a
  human clears it.
- Never apply '{{labelHook.releaseLabel}}' to any PR you open — that label arms
