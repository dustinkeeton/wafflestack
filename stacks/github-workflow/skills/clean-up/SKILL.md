...
[the gate](#the-gate) — `cleanUp.confirmGate`, or its `{{cleanUp.confirmGate.flag.off}}` /
`{{cleanUp.confirmGate.flag.on}}` tokens. Never pass it before the gate has been passed, and never
pass it on the `--run <checkpoint>` sweep, which does not touch git.

```bash
bash {{harness.skillsDir}}/clean-up/scripts/clean_up.sh            # dry run → the plan
bash {{harness.skillsDir}}/clean-up/scripts/clean_up.sh --execute  # remove worktrees, delete branches, prune
```
...
1. **Find the runs.** Checkpoints live one JSON document per run in the conventional
   `{{git.worktreesDir}}/.delegate/` directory (delegate's `delegate.checkpointDir` default):
   ```bash
   ls {{git.worktreesDir}}/.delegate/*.json
   ```
...
2. **Derive the candidates.** For each run, read `execution[]` and reconstruct
   ...
   ```bash
   node -e 'const c=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));for(const e of c.execution??[])console.log(`issue-${e.number}-${e.agent}`,e.status,e.pr??"-",c.report?"reported":"unreported")' {{git.worktreesDir}}/.delegate/<runId>.json
   ```
