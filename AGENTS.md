...
| `code-quality` | `stacks/code-quality/` | (none) | tdd, codebase-architecture, adversarial-review, dry | Cross-cutting, project-agnostic practice skills (#117; test command, tiers, module map, settings type are config). `adversarial-review` (#112) is a config-free `/adversarial-review <PR#>` hostile post-green PR review (reads `gh pr diff`, posts a severity-tagged review); auto-triggered by the github-workflow `waffle-pr-green-hook` syrup (#180). `dry` (#116) is de-duplication under the rule-of-three + semantic-sameness guardrails. |
...
PR-green hook runs; `.waffle/waffle.yaml`). The render (`.claude/agents/`, `.claude/skills/`,
