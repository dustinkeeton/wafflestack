...
| `code-quality` | `bundles/code-quality/` | (none) | tdd, codebase-architecture, electron-security-audit | Cross-cutting practice skills (desktop-app security variant). Known Obsidian-phrasing leakage — #38 follow-ups. |
| `obsidian-dev` | `bundles/obsidian-dev/` | plugin-architect | obsidian-plugin-dev | Obsidian plugin development (API, manifest, esbuild); plugin-architect is the domain architect. |
| `orchestration` | `bundles/orchestration/` | project-manager, product-manager, task-planner | delegate, audit, docs | Multi-agent orchestration; sets env `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. Roster + audit compliance are config (roster defaults: `lead-developer` architect, `security-engineer` security). Uses `requires:` for its skill deps (delegate→git-workflow+github-project-management, docs→docs-agent+docs-human). |
| `engineering-team` | `bundles/engineering-team/` | lead-developer, data-engineer, qa-engineer, devops-engineer, ux-designer, security-engineer | webapp-security-audit | Product-eng roster (browser-app security variant); lead-developer is the general architect. Slots into `orchestration`'s roster. |
...
Architect seniority rule (#38): `lead-developer` is the general architect; `plugin-architect`
