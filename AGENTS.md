...
| `installer/lib/uninstall.mjs` | `uninstall` / `reinstall` (#182), plus the pure `planUninstall` they share. **The lock is the only authority for what is ours to delete:** `planUninstall` reads `readTreeLock` (the same lock `doctor` drift-checks — the local one on an overlay machine, else the canonical one) and classifies every tracked path as `remove` (present, sha256 still matches what was rendered), `drifted` (present, hand-edited — skipped unless `force`), `absent` (a no-op), or `refused` (a key escaping `cwd` — the whole run aborts). It also computes the `.waffle/` meta to drop, the dirs the removals would leave empty (candidate dirs come from *every* tracked path so an orphan dir the consumer emptied is swept too; only a genuinely empty dir is ever pruned), the `.gitignore` lines to strip, and the ejected items to announce — all **without touching disk**, so the dry run and the real run are the same computation and cannot disagree. `uninstall` is that plan plus `fs`; it **defaults to `dryRun: true`** at the library boundary, not just behind the CLI's `--yes`. `reinstall` = uninstall(`keepConfig`+`keepLock`) + re-render, or `--clean` = uninstall-everything + `init`. |
...
```js
// render.mjs
export function renderProject({ toolkitRoot, cwd, sourceBaseDir = cwd, toolkitVersion, force = false, log, sourceCacheDir, refreshSources = false }) // → { ok, errors, warnings, written, removed, sources }  (force overrides the unmanaged-file overwrite guard; refreshSources re-fetches each git source; sourceBaseDir re-bases a relative external `source:` — only doctor --verify-render passes it)
...
// doctor.mjs
export function doctor({ cwd, toolkitVersion, allowMissing = false, verifyRender = false, toolkitRoot = null, sourceCacheDir = defaultSourceCacheDir() }) // → { ok, modified, missing, notes, attribution, allowMissing, nothingPresent, prerequisites, render }  (render = the --verify-render result, inert { evaluated:false } unless verifyRender; notes always name the rendered toolkitVersion; unmet `require` prerequisite fails ok; attribution maps external files → source label)
...
| `uninstall` | Remove the whole install, driven **entirely off the lock** (`readTreeLock` — the local lock when an overlay shaped this tree, else the canonical one). A path is deleted only if the lock tracks it **and** its sha256 still equals what was rendered (the same compare `doctor` makes); a **drifted** (hand-edited) file is skipped and reported unless `--force`; an **unreadable** one classifies as drifted too (an unverifiable hash cannot prove the file is ours — the read-only preview must classify it, never throw an `EACCES`); an **absent** one is a no-op (`--allow-missing` only silences the line). Then: remove `.waffle/` meta (config, overlay, both locks, `extensions/`) — the **one exception** to the lock invariant, since `extensions/` holds consumer-authored render *inputs* the lock cannot track, so `--keep-config` exposes the library's `keepConfig` to spare them (**and it keeps the locks with them** — `computeSelection` keeps a poured opt-in syrup selected *because its path is in the lock*, so a config without its lock is half a selection and the next `render` silently un-pours) — prune directories the removals left empty (bounded to `cwd`, only when truly empty — a `.claude/` still holding consumer files survives), strip wafflestack's own `.gitignore` lines + an orphaned `# wafflestack` marker (exact-line match; `removeGitignoreEntries`), and announce **ejected** items as project-owned and left in place. **A failed removal keeps the `.waffle/` meta**, exactly as a skip does: the lock is the only record that the file left behind is wafflestack's, and deleting it would strand the file and kill the advertised `--force` re-run (`no lock`). Retention is **one decision, made once, after the removals run** — `uninstall` reconciles `plan.lockRetained` (a plan-time *prediction*: a skip, or an explicit `keepLock`) with what execution actually did, and the skip messages read that single answer, so the report can never say "now project-owned" about a file the surviving lock still tracks. Read `lockRetained` off the **result**, not off a bare plan, when the question is "is the lock still there?" The post-run report names what was **actually** removed/pruned, not what the plan predicted. A lock key escaping `cwd` (`../…`) or an absent lock refuses the whole run: it never guesses which files are ours. **Dry run until `--yes`** (the CLI is non-interactive by design — the flag is the consent). Rejects positional refs (points at `eject`). `uninstall.mjs` (#182) |
| `reinstall` | Default = **refresh in place**: uninstall with `keepConfig`/`keepLock`, then re-render the same selection. Keeping the lock is load-bearing, not tidy — `computeSelection` keeps an already-poured opt-in (syrup) `files/` item selected *because its path is in the lock*, so dropping it would silently un-pour any syrup not also named in `include:`. Needs no `--yes`: every file it removes the render writes straight back (which is also why drifted files are re-rendered rather than skipped — a managed path is rewritten by any plain `render` regardless of hash). `--clean` = wipe to empty: uninstall everything incl. the config, then `init` a starter scaffold; no render (nothing is selected), and it **requires `--yes`**. `--force` overrides whichever safety refusal is live on the path taken (render's unmanaged-clobber guard on a refresh; uninstall's drift skip on `--clean`). `uninstall.mjs` (#182) |
...
| `help` | Print banner + usage + a one-line description of every command and flag to stdout; exit **0**. Also reachable as `--help` / `-h`, before or after a command. `cli.mjs` (#187) |
...
| Task | Command |
|------|---------|
| test | `npm test` (node:test, `installer/test/*.test.mjs`; 858 tests, 121 suites) |
