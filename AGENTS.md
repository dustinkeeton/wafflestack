...
| `installer/lib/validate.mjs` | Toolkit-developer lint: manifests, frontmatter, placeholder↔declaration sync, agent-skill + `requires:` ref integrity, config `pattern:` compilability + default-match, `files:` `targets:` names (unknown name or empty list — both would scope a payload to nothing and silently never render, #364). |
...
export function computeSelection(toolkit, project, trackedFiles = new Set()) // → { items:[{stackName,stack,kind,item}], closures, errors, targetSkipped:[{ref,targets}] }  (trackedFiles = prior lock's file paths; ungates already-installed opt-in syrup. targetSkipped = include:d files/ items every one of whose targets: is disabled — nothing renders, so render warns, #364)
...
export function skippedSyrupCompanions(toolkit, selection, targets = VALID_TARGETS) // → [{ fileRef, stackName, companions:["kind/name"…] }]  (#74: reverse the requires: edge — opt-in syrup gated out of a selection whose companion waffle IS selected; drives the install/render warning + setup update-mode flag. targets filters out syrup that could not render here anyway, #364)
...
// list.mjs
export const STATUS                                    // { CURRENT:'current', OUTDATED:'outdated', NOT_INSTALLED:'not-installed', NOT_APPLICABLE:'not-applicable' }  (NOT_APPLICABLE = target-scoped syrup this project cannot render — #364)
...
export function selectableChoices(model)               // → actionable rows (everything not `current` and not `not-applicable`; `outdated` pre-checked)  (pure)
...
| `list` | Report the whole toolkit surface per stack/item vs. what this repo has installed (`installed & current` / `out of date` / `not installed` / `not installable` — the last names a target-scoped syrup file this project cannot render, and it is excluded from the `--interactive` picker's choices, #364) as an aligned plain table (default — agent/CI-safe; ANSI only on a TTY, `--no-color` opts out). `--interactive` (real TTY both ends) drives a keypress multi-select that installs/updates the chosen refs then renders; degrades to the table without a TTY. Takes no refs. `list.mjs`, `cli.mjs:196` |
...
| `doctor` | Diff managed files vs lock; always report the rendered `toolkitVersion` + a skew note; run the selected stacks' typed `prerequisites:` checks. Exit 1 on drift OR an unmet `require`-level prerequisite (`recommend` only reports; `--allow-missing`: only modified files count as drift). `doctor.mjs:83` |
