...
export function unpouredRequiredSyrup(toolkit, selection) // → [{ ref, requiredBy, stackName }] (#371: the FORWARD direction of the #74 edge — a SELECTED item's `requires:` onto opt-in syrup nobody poured, which a stack-expanded dependent never pulls into a closure; ref is a ready `install` argument, stackName the syrup's OWN stack; skips a scoped-out dep (targetBrokenRequires' entry) and an ejected one (#502); render warns with the pour command)
...
export const STATUS                            // { CURRENT, OUTDATED, NOT_INSTALLED, NOT_INSTALLABLE, PENDING_REMOVAL } — NOT_INSTALLABLE = target-scoped syrup this project cannot render (#364); PENDING_REMOVAL = on disk + in the lock NOW and deleted by the next render, on EVERY deselection path (#371): it asks render's own prune question selection-WIDE (a live lock path NO selected item produces — never a per-row lookup, because itemOutputMatcher is stack-blind and another enabled stack may keep the path), withheld while a selection error would make render refuse; target-blind, so a fan-out prune (a selected agent's disabled-target path) is UNDER-reported, never invented
export const REMOVAL_REASON                    // { SCOPE: 'scope', DESELECTED: 'deselected' } — WHY a pending-removal row is doomed (#371), carried as row.removalReason (null on every other status): scope ⇒ its targets: are all disabled (never offered: an install persists a dead include:), deselected ⇒ its stack was disabled / include dropped (offered: re-installing it renders)
export function computeListModel({ toolkitRoot, cwd, toolkitVersion }) // → { toolkitName, toolkitVersion, lockVersion, hasLock, hasConfig, versionSkew, configError, notes, errors, stacks: [{ name, description, enabled, rows: [{ kind, name, ref, status, removalReason, optIn, targets }] }], counts }
export function formatListTable(model, { color = false } = {}) // → aligned plain-text table; color gates ANSI
export function selectableChoices(model)       // → actionable rows (not current, not scoped out; a pending-removal row iff removalReason is deselected (#371), unchecked; outdated pre-checked) (pure)
...
| `list` | Per-stack per-item state table (`current`/`outdated`/`not-installed`/`not-installable`/`PENDING REMOVAL` — the last on every deselection path, scope or stack/include, #371); plain aligned table by default (ANSI only on a TTY); `--interactive` multi-select installs + renders. Takes no refs. `list.mjs`, `cli.mjs:214` |
...
- Target scope gate (#364) — a `files:` item with `targets:` is selected only when the project
  ...
  `targetBrokenRequires` skips it. `list` asks the same prune question on EVERY deselection path
  (#371): a live lock path no selected item produces is `PENDING REMOVAL`, with a `removalReason`
  of `scope` (never offered by the picker) or `deselected` (offered — re-installing it renders);
  target-blind, so a fan-out prune is under-reported, never invented. The forward `requires:`
  edge onto un-poured opt-in syrup warns with the pour command (`unpouredRequiredSyrup`, #371) —
  `skippedSyrupCompanions` walks the edge the other way and cannot see it. The same forward
  edge onto ANY item (agent, skill, or non-opt-in file) in a stack the project does not enable
  warns too (`disabledStackRequires`, #520): expansion never pulls another stack's items in,
  so the remedy is enable the stack or `install` the item — never a silent enable. The #74
