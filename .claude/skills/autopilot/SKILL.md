...
3. **After the loop — arm or leave, then continue to Step 6.**
   - **Converged** (a round implemented 0): the PR is reviewed and clean. If **auto-merge was consented**, arm it now under the standard guardrails — `gh pr merge --auto --merge <pr>`, then on a successful arm label it `gh pr edit <pr> --add-label "waffle-auto-merged"`, and verify it took:
