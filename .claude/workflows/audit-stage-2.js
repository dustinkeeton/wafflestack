...
const AUDIT = '.claude/skills/audit/SKILL.md'
const DOCS = '.claude/skills/docs/SKILL.md'
...
const compliance = await agent(
  `Run the Toolkit integrity check pass (Task 3) exactly as \`${AUDIT}\` documents it, including its "Agent Prompts" rules.${focusLine}`,
  { label: 'compliance', agentType: 'general-purpose', schema: FINDINGS },
)
...
const changeReport = await agent(
  ...
  { label: 'docs-change-report', agentType: 'harness-architect' },
)
...
const security2 = await agent(
  ...
  { label: 'security-2', agentType: 'general-purpose', schema: FINDINGS },
)
