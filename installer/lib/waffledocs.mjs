...
function avatarsMarkdown(agents, toolkitName, git) {
  ...
  if (configured && !sharedEmail) {
    ...
    lines.push(
      ...
      '',
      'The **toolkit owner** runs `wafflestack avatars sync` once per roster change; it renders each',
      '**generated** avatar to a 512px **G-rated** PNG and, for every commit email **already verified on the',
      'Gravatar account**, uploads and assigns it over the Gravatar REST API. An agent with an authored',
      `\`identity.avatar\` (${AUTHORED} above) is listed but **not uploaded** — the sync reports it and leaves its`,
      'Gravatar registration to you. Consumers on the default `git.botEmail` inherit',
      ...
      'a clear message when it is unset. Each run uploads a **fresh** image and assigns it; older uploads',
      'remain on the Gravatar account (the assigned avatar is always the latest, so the end state is correct',
      '— prune old images in the Gravatar UI if the clutter matters).',
      '',
